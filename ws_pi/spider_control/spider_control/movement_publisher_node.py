...
class MovementPublisher(Node):
    ...
    def move(self, x, y, z, publisher):
        ...
        pose[0] -= math.radians(44)
        pose[1] -= math.radians(35)
        pose[2] -= math.radians(85)
    ...
    def tiltBase(self, alpha, beta, gamma):
        ...
        self.move(self.coordinate_back_right['x'] - deltax + deltaxy1_gamma,
                    self.coordinate_back_right['y'] + deltay + deltaxy2_gamma,
                    self.coordinate_back_right['z'] + deltaz_alpha - deltaz_beta,
                    self.publisher_back_right_)

        self.move(self.coordinate_back_left['x'] + deltax + deltaxy2_gamma,
                    self.coordinate_back_left['y'] + deltay - deltaxy1_gamma,
    ...
    def forwardMove(self, step_length, z_base):
        ...
        time.sleep(0.1)
        self.moveLift(self.publisher_back_left_, 0, step_length, -offset, -offset, lift_z)
        time.sleep(0.1)
        self.moveBase(-1.45, 2 * base_movement, z_base)
        time.sleep(0.1)
        ...
        time.sleep(0.1)
        self.tiltBase(5, -5, 0)
        self.moveLift(self.publisher_back_right_, 0, step_length, offset + 2 , -offset - 2 , 4)
        time.sleep(0.1)
        self.tiltBase(0, 0, 0)
        self.moveBase(0, 3 * base_movement, z_base)
    ...
    def timer_callback(self):
        ...
        if self.rising_edge(2):  # TRIÁNGULO
            self.X_flag = False
            self.O_flag = False
            self.T_flag = True
            self.C_flag = False

        if self.rising_edge(3):  # CUADRADO
            self.X_flag = False
            self.O_flag = False
            self.T_flag = False
            self.C_flag = True
        ...
        if self.T_flag: # TRIANGLE button
            self.moveLift(self.publisher_back_right_, 0, 5, 0 , 0 , 20) 
            self.T_flag = False
